...
import { fetchTokenURI, fetchTokenURI2 } from "./utils/erc721";

// Constants
let ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";
let ITEM_ORDERS = "0x64caed7ccc8138be20299cdc37ec1e81900147b4";
let ITEM_HELPER = "0xe57baf27e8aa82a66d459f674c6c31ffb0a42886";
let ITEM_HELPER2 = "0xa229768ab302fc0881ab0b3738e47b4e577e2d3c";
let NFT_ORDERS = "0x198bb79ecbebd04394a96e0f959594a3f81b6050";
let NFT_HELPER = "0x5b5a9278eab3c7fb3f3d7d3c5abe0397fbf45e4d";
let NFT_HELPER2 = "0xcb2732536345ff3d91e03e7bcc2e660564c243be";
let PAYWALL_ORDERS = "0x58060f1714e2ba8fc8d94e33ba647a723d4b0347";
let PAYWALL_HELPER2 = "0x4cce9d3a7895d62a6336017fe7354e17d346f627";
let NFTICKET_HELPER2 = "0xab45391c5ee3270880565e2e9b6ba1d01a4a2ccf";
...
export function handlePaywallAskCancel(event: PaywallAskCancel): void {
    let isPaywall = event.params.sender.equals(Address.fromString(PAYWALL_ORDERS))
...
export function handleUpdateMiscellaneous(event: UpdateMiscellaneous): void {
    ...
    if (event.params.idx.equals(ZERO_BI)) { // partnership between 2 paywalls
    ...
    if (event.params.sender.equals(Address.fromString(PAYWALL_HELPER2)) || event.params.sender.equals(Address.fromString(PAYWALL_ORDERS))) {
    ...
    if (event.params.sender.equals(Address.fromString(ADMIN_ADDRESS))) {
        if (event.params.idx.equals(TEN_BI)) {
            ...
            note.collection = event.params.collectionId.toString();
            if (event.params.paramValue2.equals(ONE_BI)) {
                note.paywall = event.params.collectionId.toString() + "-" + event.params.paramValue5;
            } else {
                note.item = event.params.collectionId.toString() + "-" + event.params.paramValue5;
            }
            note.save()
        }
    }
    if (event.params.idx.equals(ELEVEN_BI)) {
        let eCollectible = Collectible.load(event.params.collectionId.toString() + '-' + event.params.paramName + '-' + event.params.paramValue2.toString());
        if (eCollectible === null) {
            eCollectible = new Collectible(event.params.collectionId.toString() + '-' + event.params.paramName + '-' + event.params.paramValue2.toString());
            eCollectible.createdAt = event.block.timestamp;
            eCollectible.nfticketId = event.params.paramValue3;
            eCollectible.tokenId = event.params.paramValue2;
            eCollectible.userAddress = event.params.paramValue4.toHexString();
            eCollectible.minterAddress = event.params.sender.toHexString();
            eCollectible.collection = event.params.collectionId.toString();
            let uri = fetchTokenURI(event.params.sender, event.params.paramValue2);
            if (uri !== null) {
                log.warning("uri1===============> - #{}", [uri]);
                eCollectible.metadataUrl = uri;
