...
export function updateCollectionDayData(collectionId: BigInt, newVolumeInBNB: BigDecimal, event: ethereum.Event): void {
  let timestamp = event.block.timestamp.toI32();
  let dayID = timestamp / 86400;
  let dayStartTimestamp = dayID * 86400;
  let ID = dayID.toString() + "-" + collectionId.toString();
  ...
  if (collectionDayData === null) {
    ...
    collectionDayData.collection = collectionId.toString();
